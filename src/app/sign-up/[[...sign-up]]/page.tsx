...
import { Mail, Lock, Sparkles, ArrowRight, Globe, GitBranch } from "lucide-react";
...
export default function Page() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (createError) {
        setError(createError.message || "Unable to sign up.");
        return;
      }
      ...
      if (signUp.status === "complete") {
        ...
        if (finalizeError) {
          setError(finalizeError.message || "Unable to finish sign up.");
        }
        return;
      }
  ...
  async function handleOAuth(strategy: "oauth_google" | "oauth_github") {
    ...
    try {
      ...
      setError(err?.errors?.[0]?.message || "Unable to start OAuth sign up.");
      setLoading(false);
    }
  }
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-slate-50 text-slate-900 dark:bg-[#0A0A0B] dark:text-white">
      <div className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute -top-48 left-1/2 h-[520px] w-[520px] -translate-x-1/2 rounded-full bg-purple-500/20 blur-[190px] dark:bg-purple-500/10" />
        <div className="absolute -bottom-48 right-0 h-[440px] w-[440px] rounded-full bg-blue-500/20 blur-[190px] dark:bg-blue-500/10" />
      ...
      <main className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-8 px-6 pb-16 pt-6 lg:grid-cols-[0.45fr_0.55fr]">
        <div className="hidden lg:flex flex-col justify-center rounded-3xl border border-slate-200 bg-white/80 p-10 shadow-2xl backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.03]">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-500 dark:text-slate-400">Sign up</p>
          <h1 className="mt-3 text-4xl font-extrabold tracking-tight">
            Start your CodeStorm account
          </h1>
          <p className="mt-4 text-sm text-slate-600 dark:text-slate-400">
            Join the premium cyber workspace with seamless Clerk authentication.
          </p>
          ...
          <div className="mt-8 space-y-3">
            <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
              ...
              A clean, glassy interface in dark mode.
            </div>
            <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
              <ArrowRight className="h-4 w-4 text-blue-500 dark:text-blue-300" />
              Land right on the dashboard after signup.
            </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white/90 p-8 shadow-2xl backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.03]">
          <div className="mb-6">
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-500 dark:text-slate-400">Create account</p>
            <h2 className="mt-2 text-2xl font-bold">Join CodeStorm</h2>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
              Use your email and a secure password to begin.
            </p>
          </div>
          ...
          <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
            ...
            <Link href="/sign-in" className="font-semibold text-slate-900 hover:text-blue-600 dark:text-white dark:hover:text-blue-200">
              Sign in
            </Link>
          </p>
        </div>
